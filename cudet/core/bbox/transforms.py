...
def bbox2delta(proposals, gt, means=[0,0,0,0],stds=[1,1,1,1]):
    """
    Perform bbox style to delta.This can be find in paper(Cascade R-CNN).
    delta:(-1,4) delta=yx-bx/bw
    delta_dot=delta-mean/stds
    :param proposals: (-1,4) xmin,ymin,xmax,ymax
    :param gt:
    :param means:
    :param stds:
    :return:
...
def delta2bbox(rois,
    ...
    max_ratio = np.abs(np.log(wh_ratio_clip))   # 1.79
...
def bbox_flip(bboxes, img_shape):
    ...
    if isinstance(bboxes, torch.Tensor):
        ...
        return mtcv.bbox_flip(bboxes, img_shape)
