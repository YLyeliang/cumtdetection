...
def anchor_target(anchor_list,
    ...
    # anchor number of multi levels,low level have more anchors
    ...
    # split targets to a list w.r.t. multiple levels  (with respect to )
...
def images_to_levels(target, num_level_anchors):
    ...
    num_level_anchors(list[]):each elements are anchor number of corresponding level.
...
def anchor_target_single(flat_anchors,
    ...
    if len(pos_inds) > 0:
        ...
        if gt_labels is None:
            ...
            label_weights[pos_inds] = gt_labels[sampling_result.pos_assigned_gt_inds]
        if cfg.pos_weight <= 0:
...
def anchor_inside_flags(flat_anchors, valid_flags, img_shape,
                        allowed_border=0):
    # set valid regions under allowed_border to 1.Means flags.
...
def unmap(data, count, inds, fill=0):
    ...
    (of size count)"""
    if data.dim() == 1:
        ret = data.new_full((count,), fill)  # have same type as data.
        ret[inds] = data
    else:
