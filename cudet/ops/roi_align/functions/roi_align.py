...
class RoIAlignFunction(Function):
    ...
    def forward(ctx,features,rois,out_size,spatial_scale,sample_num=0):
        ...
        batch_size,num_channels,data_height,data_width = features.size() # NCHHW # feature_size
        num_rois = rois.size(0)

        output = features.new_zeros(num_rois,num_channels,out_h,out_w) # NCHW  out_size_h
        if features.is_cuda:
